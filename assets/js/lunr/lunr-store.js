var store = [{
        ...
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        ...
        "title": "C++ 中的 stack",
        "excerpt":"要真正理解栈这种结构，意味着我们要剥离事物表象，追究其最本质的问题，即我们到底面临什么样的数据处理需求，才导致了 “栈” 这种结构的诞生？ 我们不需要把栈仅仅看作是教科书上的定义，而是把它看作是为了解决特定时序问题而设计的一种 “受限” 模型。 1. 什么是栈？ 从本质上来说，计算机处理任务通常有两种模式： 并行/随机：所有任务都在桌面上铺开，想拿哪个拿哪个（类似数组或哈希表）。 嵌套/依赖：任务 B 必须依赖任务 A 的环境，任务 C 又依赖 B。要完成 A，必须先挂起 A 去做 B；要做 B，必须先挂起 B 去做 C。 栈（Stack）就是为了解决第二种模式而诞生的。 1.1 核心定义：LIFO 栈是一种线性表，但它是一种受限的线性表。它的核心逻辑是后进先出（Last In, First Out - LIFO）。 你可以把它想象成叠盘子： 压入（Push）：你只能把新盘子放在最上面。 弹出（Pop）：你只能取走最上面的盘子。 查看（Top/Peek）：你只能看到最上面盘子的花纹，无法直接看到底部的盘子。 生活中的栈模型：想要拿到底部的盘子，必须先移走顶部的盘子 1.2 为什么它是 “受限” 的？ 数组（Array）允许你访问 index[0] 也可以访问 index[100]。栈为什么要把这种能力 “阉割” 掉？...","categories": ["Note"],
        "tags": ["C++"],
        "url": "/note/stack-in-cpp/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        "title": "如何将 WSL2 镜像无损迁移至非系统盘",
        "excerpt":"1 引言 随着开发环境的日益庞大，默认安装在 C:\\ 盘的 WSL2 (Windows Subsystem for Linux) 镜像体积也会不断膨胀。本文将分享如何通过命令行操作，将 WSL 迁移至 E:\\ 盘乃至其他盘符，彻底释放系统盘空间。 1.1 笔者环境 名称 版本 Windows 11 Pro 25H2 26200.8037 PWSH 7.5.5 WSL 2.6.3.0 Ubuntu 22.04 LTS Ubuntu 24.04 LTS 注意：这里的 PWSH 并不是指 Windows 系统内置的经典 PowerShell，而是跨平台的开源项目 PowerShell Core。下文中使用 PowerShell 指代系统默认版本，PWSH 指代开源版本。 2 准备工作 2.1 确认当前安装状态 如果你还没有安装镜像，或想查看当前支持的发行版，可以使用以下命令：...","categories": ["Tutorial"],
        "tags": ["Windows","WSL2","Ubuntu"],
        "url": "/tutorial/wsl2-image-migration-guide/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        "title": "ARMv7-A 指令体系全解：数据处理、访存、分支与同步",
        "excerpt":"I.MX6ULL 的核心是 ARM Cortex-A7，它基于 ARMv7-A 架构。 要真正理解指令集，就不能只是背诵汇编手册，而是要回答一个核心问题：如果不看代码，只看电路，CPU 在做什么？ 本质上，每一条指令都是对 CPU 内部寄存器状态或外部总线（内存/外设）的一次原子操作。 总线（Bus）通俗理解就是主板上的 “高速公路”。它的硬件本质则是一组并排的导线，通过这组导线，连接 CPU 内部各部分或连接 CPU 和外设。 总线分为数据总线（传数据）、地址总线（告诉内存我要读哪一个单元）和控制总线（读还是写）。 原子操作（Atomic Operation）通俗理解就是，要么做完，要么完全不做，中间绝不允许被打断的操作。 在 ARM 中，单条读写指令通常是原子的，但 “读-改-写” 三步操作默认不是原子的，需要特殊指令（ldrex / strex） 保证。 1. 物理本质 在 ARM 状态下（Cortex-A7 上电默认状态），每一条指令都是严格的 32 位（4 字节）二进制码。 与 x86（变长指令）不同，ARM 的定长指令让译码器设计更加简洁高效。一条典型的 ARM 指令在物理电路上被切割为以下结构： Instruction = [Codition Code (4 bits)] + [OpCode...","categories": ["Note"],
        "tags": ["ARM","Assembly","i.MX6ULL"],
        "url": "/note/armv7-a-isa/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        "title": "GNU 汇编（GAS）通用语法与伪指令指南",
        "excerpt":"由于机器指令及助记符高度依赖于芯片的具体架构，本文不讨论特定指令集的全部细节，而是聚焦于 GAS（GNU Assembler）通用的伪指令系统及其基本语法规则。 1. 汇编器的本质与工作流 GNU Assembler（简称 as）的核心工作流可以用以下公式概括： $$ \\text{源代码 (.s)} \\xrightarrow{\\text{汇编器 (as)}} \\text{可重定位目标文件 (.o)} $$ 生成的 .o 文件主要由 Section（段）和 Symbol Table（符号表）组成。我们常用的 GAS 伪指令（Directives/Pseudo-ops），其根本目的就是为了控制这两个核心要素的生成。 从本质上来说，GNU Assembler 的核心职能并非简单的 “代码翻译”，而是内存空间的规划与符号地址的解析。 汇编器内部维护着一个至关重要的变量：地址计数器（Location Counter，通常记为 .）。当汇编器读取源代码时，它实际上是在执行一个状态机循环： 解析伪指令：根据指令调整地址计数器的位置（例如跳转到新的段、预留空白空间）或修改符号的属性（如设置为全局可见）。 发射字节流：将翻译好的机器指令或定义的静态数据 “填充” 到地址计数器当前指向的内存单元，随后让计数器自增，指向下一个空闲位置。 构建符号表：记录所有标号（Label）对应的最终物理地址或相对于段的偏移量。 因此，当我们编写汇编代码时，实际上是在做两件事： 填充内存：明确告诉汇编器，在当前的内存地址上应当放置什么数据（注意：机器指令在内存中本质上也是数据）。 标记位置：给特定的内存地址起一个人类可读的名字（Label/Symbol），以便后续进行跳转或读写操作。 2. 指令语法 GAS 语法的核心难点在于不同架构下对操作数方向的处理，以及如何消除符号的歧义。 2.1 注释风格 不同的 CPU 架构在 GAS 中通常使用不同的单行注释符号。这里仅列举常见架构的规范，更详细的列表可查阅...","categories": ["Note"],
        "tags": ["ARM","RISC-V","Assembly","i.MX6ULL","GNU"],
        "url": "/note/gnu-assembly/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        "title": "英语语法综述",
        "excerpt":"我们为什么学语法？ 当然是为了考高分啊😂 其实，所有的英语语法都只有一个目的：造句。 英语和中文一样，我们说话写文章有的时候用短句子，有的时候用长句子。不同的短句子又可以组合成新的长句。但这里说的是句子的长度，并不适合语法讨论。 我们只有把句子拆开来，直到拆到不能再继续拆了，否则句意会不完整的 “基本句”，才好研究语法。因为如果 “基本句” 研究透了，长句子就是它们的组合。 1. 简单句（Simple Sentence） 我们把这样的不能再拆的 “基本句” 称作简单句（Simple Sentence）。而这样的简单句，除去那些感叹句（“嗯”，“哦”，“啊”等）、省略句、（向整句提问的）问句，几乎全部都是在说： 什么 + 怎么样 绝大多数句子粗略地分一共就只有这两个部分，而这个 “什么” 和 “怎么样” 分别对应了两个最基本的句子成分： “什么”： 主语（Subject）。 “怎么样”：谓语（Predicate）。 换句话说，几乎所有的英语句子结构都是： 主语 + 谓语 这里的主语一般是人或物，不管是抽象还是具体。而这里的谓语则是指一个广义的动作或发生的事情。 这个 “动作” 不是我们平时狭义说的要动起来的 “动作”，而这个广义的 “动作” 也就是我们语法上说的动词（Verb）。 2. 谓语动词 序号 动词类型 解释 1 不及物动词 可以独立完成的动作 2 单及物动词 有一个动作的承受者 3 双及物动词...","categories": ["Note"],
        "tags": ["English"],
        "url": "/note/overview-of-english-grammar/",
        "teaser": "/assets/images/english_grammar_system.png"
      },{
        "title": "从 FHS 到 Linux 运行时目录体系",
        "excerpt":"要真正理解 Linux 的 FHS (Filesystem Hierarchy Standard，文件系统层次结构标准)，不能只靠死记硬背“哪个目录放什么文件”。更有效的办法，是剥开表象，回到操作系统设计的起点，看看在构建一个多用户、网络化、可扩展的系统时，我们究竟面临了哪些根本问题，以及 FHS 又是如何作为一套“解决框架”出现的。 不过，本文并不准备停在 FHS 的条文本身。更准确地说，行文会分成两步：先把 FHS 作为规范 讲明白，再顺着这套规范继续往外延伸，看看现代 Linux 是如何在这棵目录树上接入运行时机制、容器隔离、嵌入式约束和内核接口的。也正因为如此，标题与其叫“Linux 的 FHS”，不如更直接一点，叫“从 FHS 到 Linux 运行时目录体系”。 推演起点：我们到底要存储什么？ 假设你从零开始设计一个操作系统。系统需要运行，用户需要存数据，程序需要产生日志。如果你把所有东西都塞进一个大文件夹里，系统马上就会面临灾难： 权限混乱： 普通用户可能会不小心删掉系统的核心启动文件。 状态混淆： 你无法区分哪些文件是只读的系统核心，哪些是随时变动的日志，导致备份极其困难。 共享灾难： 如果你想在局域网内让多台机器共享一套程序，你无法分离 “机器特有的配置” 和 “可以共享的程序本体”。 因此，FHS 的本质，是基于数据属性的二维分类矩阵。Linux 的文件系统布局，本质上就是围绕着这两个核心维度展开的： 维度一：是否可变 (Static vs. Variable) 静态数据 (Static)： 系统运行期间不需要修改的文件（如可执行程序、库文件、默认系统配置）。 可变数据 (Variable)： 系统运行期间会不断变化的文件（如日志、用户数据、临时文件、缓存）。 维度二：是否可共享 (Shareable vs....","categories": ["Note"],
        "tags": ["Linux"],
        "url": "/note/fhs-of-linux/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        "title": "Tmux 使用笔记",
        "excerpt":"为什么我们需要 tmux？ 1. 终端的物理与逻辑基础 在计算机世界里，我们通过 “终端（Terminal）” 与 “Shell（如 bash, zsh）” 进行交互。 当你打开一个终端窗口，或者通过 SSH 连接到远程服务器时，操作系统为你分配了一个 TTY（伪终端），并在这个 TTY 上启动了一个 Shell 进程。你在终端里运行的所有程序（跑脚本、开服务器、编辑代码），都是这个 Shell 的子进程。 2. 致命的耦合（痛点所在） 这就产生了一个致命的 “生命周期耦合” 问题： 规则：当你的终端窗口关闭，或者由于网络波动导致 SSH 连接断开时，系统会向该 TTY 上的 Shell 发送一个 SIGHUP（挂断信号）。 结果：Shell 收到信号后会立即终止，并带着它所有的子进程一起退出。 痛点：如果你正在服务器上训练一个需要跑 3 天的深度学习模型，或者正在编译一个巨大的项目，只要你的网络断开一秒钟，或者你不小心关掉了窗口，前功尽弃。 3. 需求推演 为了解决这个问题，我们需要在 “显示界面（你的屏幕/SSH连接）” 和 “后台进程（Shell 及你的任务）” 之间，插入一个中间层。 这个中间层需要具备两个核心能力： 进程解耦（持久化）：它可以接管所有的 Shell...","categories": ["Note"],
        "tags": ["Linux"],
        "url": "/note/note-of-tmux/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        "title": "BibTeX 与 BibLaTeX 入门：.bib 文件结构、引用与编译流程",
        "excerpt":"最近恰逢本科阶段的倒数第二个课程设计，初步估计又得三四十页，并且老师不发模板只发排版要求，美其名曰让我们提前适应毕业论文写作（但是我记得毕业论文也是有 Word 模板的吧😂）。 我并非是某一方的狂热爱好者，在我看来两者都是工具，只要能解决我的问题，那就是好工具。显然这种比较专业的排版需求还是使用 LaTeX 比较合适，而我发现我对 LaTeX 的引用数据库还是不太了解，就大概学习了一并下整理了这篇博客，不一定足够专业，但是应该能够应付我的课程设计了。 1. .bib 文件的基本结构 .bib 文件可以理解成一个文献数据库，由多条文献条目组成。每条条目一般是下面这种形式： @entrytype{citekey, field1 = {value1}, field2 = {value2}, ... } @entrytype：文献类型，例如 @article、@book、@inproceedings。 citekey：引用键（citation key），在正文中通过 \\cite{citekey} 调用。 field = {value}：字段和值，如 author、title、year。 1.1 常见条目类型与常用字段 条目类型 说明 常用字段 @article 期刊论文 author, title, journal, year, volume, number, pages, doi @inproceedings 会议论文 author,...","categories": ["Note"],
        "tags": ["LaTeX"],
        "url": "/note/bibtex-and-biblatex-guide/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        "title": "设备树学习笔记：从基础语法到 DTB",
        "excerpt":"0. 引言：为什么我们需要设备树？ 想象这样一个场景：你的电脑开机，操作系统启动，它需要知道自己运行在什么硬件之上：有几颗 CPU、多大内存、串口在哪里、网卡是什么型号。操作系统如何获取这些信息？ 答案是……看情况。 0.1 自发现 vs 非自发现总线 现代计算机系统中，总线大致分为两类。 自发现总线（Self-Discoverable Bus）：PCIe、USB 这类总线，操作系统可以向总线发送探测请求，插在总线上的设备会主动回答 “我在这里，我是某某设备”。插上一个 USB 键盘，系统立刻就能知道这个键盘的存在及其设备描述符。这是一种运行时动态发现机制。 非自发现总线（Non-Discoverable Bus）：I2C、SPI、CAN、系统总线（Processor Bus）等则完全不是这样。在这些总线上，CPU 无法主动 “问” 谁连接在上面。它只知道有一组内存映射的寄存器地址，但对于这些地址背后是什么设备、什么型号、如何配置，CPU 一无所知。这些信息必须由人为提供。 嵌入式系统大量使用非自发现总线。一个典型的 ARM SoC 上，UART、I2C 控制器、GPIO 控制器、定时器都是通过内存映射连接到处理器总线的，没有任何动态发现机制。 0.2 历史的教训：Linus 的愤怒 在设备树出现之前，Linux 对这些非自发现硬件的处理方式简单粗暴：硬编码。 ARM 架构的 Linux 内核曾经有一个庞大的 arch/arm/mach-* 目录，里面为每一款开发板都写了一份 C 代码，直接在内核源码中描述板上有什么设备、基地址是多少、中断号是多少。如果你买了一块新的开发板，就需要在内核里添加一份新的 board-*.c 文件。 这导致了几个严重问题： 内核膨胀：同一份内核镜像无法在多块不同板子上运行。每块板子都需要编译一个专用内核。 代码冗余：不同板子之间共享同一个 SoC，但因为板上外设组合不同，代码大量重复。 维护灾难：2011 年左右，Linux...","categories": ["Note"],
        "tags": ["Linux"],
        "url": "/note/devicetree-study-notes/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
        "title": "LaTeX 项目结构与 latexmk 配置",
        "excerpt":"目录结构 C:. │ .gitignore │ .latexmkrc │ LICENSE │ main.tex │ refs.bib │ ├─contents ├─docs ├─figs └─settings format.tex packages.tex main.tex：入口文件，只负责把各部分拼起来。 refs.bib：参考文献数据库。用 BibTeX 格式存储文献条目，正文里用 \\cite{key} 引用。配合 biblatex 使用时，在 format.tex 里用 \\addbibresource{refs.bib} 指定路径。 contents/：章节内容，按章（book/report 类）或按节（article 类）建子文件夹。 figs/：图片资源。配合 \\graphicspath 使用，正文里直接写文件名就行。 docs/：项目文档，比如编译好的 PDF、使用说明等。 settings/packages.tex：宏包导入。 settings/format.tex：格式配置（页面布局、标题样式、字体等）。 .latexmkrc：latexmk 的编译配置，后面会详细讲。 .gitignore：LaTeX 项目的忽略模板可以参考 github/gitignore/TeX.gitignore。注意官方模板里没有 build/ 目录，如果你像我一样把编译产物输出到 build/，记得手动加上。 packages.tex...","categories": ["Note"],
        "tags": ["LaTeX"],
        "url": "/note/latex-project-structure/",
        "teaser": "/assets/images/logo-teaser.jpg"
      },{
    ...
    "title": "Portfolio",
    "excerpt":" ","url": "https://bfmhno3.github.io/portfolio/"
  },{
    "title": "Posts by Tag",
    ...
    "title": "WSL2",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/wsl2/"
  },{
    "title": "Ubuntu",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/ubuntu/"
  },{
    "title": "ARM",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/arm/"
  },{
    "title": "Assembly",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/assembly/"
  },{
    "title": "i.MX6ULL",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/i-mx6ull/"
  },{
    "title": "RISC-V",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/risc-v/"
  },{
    "title": "GNU",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/gnu/"
  },{
    "title": "Linux",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/linux/"
  },{
    "title": "LaTeX",
    "excerpt":"","url": "https://bfmhno3.github.io/tag/latex/"
  },{
    "title": "Note",
    ...
    "title": "Better Mistakes - page 5",
    "excerpt":"","url": "https://bfmhno3.github.io/page/5/index.html"
  },{
    "title": "Better Mistakes - page 6",
    "excerpt":"","url": "https://bfmhno3.github.io/page/6/index.html"
  }]
