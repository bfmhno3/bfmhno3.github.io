...
要真正理解 Linux 的 **FHS (Filesystem Hierarchy Standard，文件系统层次结构标准)**，我们不能仅仅去死记硬背哪个目录下存放了什么文件。相反，我们需要剥开表象，回到操作系统设计的起点，看看在构建一个多用户、网络化、可扩展的系统时，我们面临了哪些根本性问题，以及 FHS 是如何作为一套 “解决框架” 应运而生的。
...
有了上面的维度拆解，我们再来看 Linux 的根目录 `/` 下的各个分支，一切就变得顺理成章了。
...
- **`/etc` (Etcetera / 配置文件)：** 本机的“规则字典”。存放所有主机特有的（不可共享的）、静态的系统配置文件。**原则：** 这里面应该全是纯文本文件，绝不能有可执行二进制文件。
...
**`/usr` (Universal System Resources / 以前指 User)：** 系统的 “第二层级”。你可以把它理解为 Windows 下的 `C:\Windows` 和 `C:\Program Files` 的结合体。它是只读的，包含了庞大的软件资源。
...
上一篇的重点在于搭建 FHS 背后“可变/静态”与“共享/不可共享”的底层逻辑框架。顺着这条主线继续往下推，会发现还有一些对开发者，特别是做 C/C++ 编译、驱动、工具链和底层系统开发的人来说，极其关键的目录，值得单独补齐。
...
- **`/usr/share` (架构无关的共享数据)**
  ...
  - **本质：** 无论你是 x86 还是 ARM，这里的数据都是通用的。这包括 `man` 帮助文档（`/usr/share/man`）、时区数据（`/usr/share/zoneinfo`）、以及构建工具的配置文件（例如 CMake 的默认模块通常存放在 `/usr/share/cmake/Modules/` 下）。
- **`/usr/libexec` (系统内部调用的可执行文件)**
  - **属性：** 静态，可共享。
  - **本质：** 这里存放的二进制程序**不是给用户在终端里敲击运行的**，而是给其他程序调用的后台工具。比如系统的认证模块守护进程、某些网络配置的辅助脚本等。它们没有被放进 `/usr/bin`，就是为了防止污染用户的环境变量 `PATH`。
...
- **`/run` (运行时状态数据 - 现代 tmpfs)**
  - **属性：** 实时动态，不可共享，**纯内存存储**。
  - **本质：** 记录系统自本次开机以来的状态数据（如守护进程的 PID 文件、Socket 文件等）。在早期的 FHS 中，这些数据放在 `/var/run`。但因为 `/var` 通常在硬盘上，读写慢且可能在启动早期尚未挂载，所以现代 Linux 创建了 `/run` 并将其挂载为 `tmpfs`（内存文件系统）。每次重启，这里自然清空。
- **`/dev/shm` (POSIX 共享内存)**
  - **属性：** 实时动态，纯内存存储。
...
- **`/srv` (服务数据 Service Data)**
  ...
  - **本质：** 存放本机对外提供服务的数据。比如你在这台机器上跑了一个静态博客系统或 Web 服务器，按照 FHS 规范，网站的根目录数据（如 HTML、CSS、Markdown 渲染后的文件）不应该放在 `/home` 或 `/var` 下，而应该放在 `/srv/www/` 或 `/srv/http/` 中。
...
把这些补充的目录放回之前的二维分类矩阵中，整个 FHS 就严丝合缝了：

|数据属性|静态 (Static) <br> *系统运行不改变，可只读挂载* |可变 (Variable) <br> *系统运行持续写入*|实时内存抽象 <br> *不占硬盘，描述状态或提供机制*|
|:---|:---|:---|:---|
|**不可共享 (Unshareable)** <br> *特定于本机*|`/boot`, `/etc`, `/lib/modules`|`/var`, `/lost+found`, `/srv`|`/dev`, `/proc`, `/sys`, `/run`|
|**可共享 (Shareable)** <br> *可跨主机复用*|`/usr`, `/usr/include`, `/usr/src`, `/usr/share`, `/usr/libexec`, `/opt`|`/home`, `/tmp`|`/dev/shm`|
...
- **文件系统映射：** * **`/snap` 或 `/var/lib/flatpak`：** 现代桌面 Linux 为了解决这个问题，干脆放弃了共享库。一个 App 把自己需要的所有底层库全部打包在一起，挂载成一个只读的虚拟磁盘。
...
- **底层逻辑：** 早期的 Linux 系统里，各种软件喜欢在用户的 `/home/user/` 目录下乱拉屎，随便建各种隐藏文件（比如 `.bashrc`, `.viminfo`, `.nvm`）。这导致用户的家目录极其混乱。为了解决这个问题，freedesktop.org 提出了 XDG 标准，规定了用户级应用的数据必须分类存放。
- **文件系统映射：**
...
- **文件系统映射：**
  ...
  - **原理：** 这里并不是简单的磁盘共享。Windows 使用的是 DrvFs 插件，底层通过 **9P 网络协议**（一个源自贝尔实验室 Plan 9 系统的分布式文件系统协议）来实现跨 OS 的文件访问。它需要在 Linux 的 POSIX 权限模型（rwx，所有者/组）和 Windows 的 ACL（访问控制列表）之间做复杂的实时翻译。这就是为什么在 WSL2 里跨文件系统（比如在 Linux 里去编译 `/mnt/c` 下的 C++ 项目）进行高频 I/O 操作会比较慢的原因 —— 每一次读写都要经过协议层的翻译。
